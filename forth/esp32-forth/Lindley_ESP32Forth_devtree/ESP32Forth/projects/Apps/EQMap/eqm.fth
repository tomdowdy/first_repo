...
: formatDisplayTime ( -- ) 
  ...
  now toLocal >r r@ hourFormat12_t . cr
...
: AcqDisplay ( -- )
  ...
  if
    ...
    if 
      ...
      LON_BUF getString FP.fromS" lonToMapX to scnX
      LAT_BUF getString FP.fromS" latToMapY to scnY

      \ Magnitude is rounded to nearest int value
      MAG_BUF getString FP.fromS"R FP.toInt to mag
