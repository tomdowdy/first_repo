...
: _getPixel { disp x y } ( disp x y -- f )
  ...
  x y getPixelIndex FRAMEBUFFER c@ and 0<>
;

\ Get a pixel's state (off/on) from selected display
: getPixel { disp x y } ( disp x y -- f )
\  ." GP _disp: " disp . ."  _x: " x . ."  _y: " y . cr
  x y transform
  disp -rot _getPixel
;
...
\ Erase contents of frame buffer
: clearFrameBuffer ( -- )
