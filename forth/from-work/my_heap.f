...
: adjust.hndls swap negate end.heap.c 
	get.hndl.ptr 2+ do i @ 2 pick > 
	get.hndl.ptr i @ - 0> and 
	if i over swap +! then 
	2 +loop 2drop ;
\ : get.hndl get.free.hndl ?dup 0= if get.hndl.ptr -2 bump.hndl.ptr else dup @ update.free.hndl then ;
: get.hndl ( -- hndl ) free.hndl.ptr @ ?dup 0= if hndl.ptr @ -2 hndl.ptr +! else dup @ free.hndl.ptr ! then ;
...
: alloc ( size -- hndl or 0; usage size alloc name ) ?neg ?full 
	get.hndl
	get.heap.ptr 2+ over ! over
	get.heap.ptr ! swap 2+ 
	bump.heap.ptr ;
: size? ( hndl -- size ) @ 2- @ ;
