...
: cell+m ( n -- n+2) compile 2+ ; immediate
...
: .$const ( $Caddr -- ) \ "display string constant"
    ...
    cell+m dup (lenOf$) swap cell+m swap type ;
...
: :=" ( $Caddr tib:"string" -- ) \ "assign string constant"
    ...
    nip 2dup swap cell+m !
    >r [ 2 cells ] literal + r> r> -rot cmove ;

: ($") ( addr len -- ) ( ss: -- str )
    \ run-time action for $" (see below)
    dup ($rUp) cell+m (set$SP)
    dup ($sp@) !  ($sp@) cell+m swap cmove  ($depth+) ;
...
: >$ ( $Caddr -- ) ( ss: -- str) \ "string constant to string stack"
    ...
    \ e.g. fred >$
    cell+m dup (lenOf$) swap cell+m swap ($") ;
...
: pick$ ( n -- ) ( ss: -- strN) \ "pick string"
    ...
    (addrOf$) dup (lenOf$) swap cell+m swap ($") ;
...
: >$const ( $Caddr -- ) ( ss: str -- ) \ "to string constant"
    ...
    ($sp@) dup (sizeOf$) r> cell+m swap cmove drop$ ;
...
: +$ ( -- ) ( ss: s1 s2 -- s2+s1) \ "concatenate strings"
    ...
    \ displays "redblue"
    depth$ 2 < if 9903 (throw) then
    1 (addrof$) cell+m  here   1 (addrof$) (lenof$)  cmove
    ($sp@) cell+m   1 (addrof$) (lenof$) here +  len$ cmove
...
: find$ ( offset -- pos|-1 ) ( ss: s1 s2 -- s1) \ "find string"
    ...
    1 (addrOf$) cell+m + ($temp2) !    ($sp@) cell+m ($temp3) !
...
: .$ ( -- ) ( ss: str -- ) \ "display string"
    ...
    depth$ 0= if 9902 (throw) then
    ($sp@) cell+m ($sp@) (lenOf$) type  drop$ ;

: rev$ ( -- ) ( ss: s1 -- s2 ) \ "reverse string"
    \ reverse top string on string stack.
    depth$ 0= if 9902 (throw) then
    ($sp@) dup cell+m >r  (lenOf$)  r> swap here swap cmove
    ($sp@) (lenOf$) here 1- +
    ($sp@) cell+m  dup ($sp@) (lenOf$) +   swap do
...
: ltrim$ ( -- ) ( ss: s1 -- s2 ) \ "left trim string"
    ...
    0  r> here cell+m dup >r +  r> do
        i c@ bl = if 1+ else leave then loop
    dup 0> if
        >r  ($sp@) (lenOf$)  drop$
        here cell+m r@ +  swap r> -  ($")
    else drop then ;
...
: replace$ ( -- pos ) \ "replace string"
    ...
    0 find$ dup ($temp0) ! -1 > if
        ($sp@) cell+m  here  ($temp0) @ cmove
        1 (addrOf$) cell+m   here ($temp0) @ +
...
: ucase$ ( -- ) ( ss: str -- STR) \ "to upper case"
    ...
    \ to upper case.
    depth$ 1 < if 9902 (throw) then
    ($sp@) dup (lenOf$) + cell+m  ($sp@) cell+m  do
...
: ==$? ( -- flag ) ( ss: -- ) \ "are strings equal?"
    ...
    len$  1 (addrOf$) (lenOf$) = if
        1 (addrOf$) cell+m \ point to first char of string 1
        ($sp@) cell+m  dup len$ + swap  do
