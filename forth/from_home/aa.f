...
: here- here cell - ;
: pop here cell - @ cell negate dp +! ; \ dp +! = allot
: npush 0 do , loop ;
: npop 0 do pop loop ;
...
: peek here cell - @ ;
: allot- negate dp +! ;
: ddrop cell negate dp +! ; \ dictionary (dstack) drop
...
\ Same as nmap but use the [. and .] to create your xt.
...
\ do a ,reset when done with dstack to recover memory used for counter.
\ slower than pop-n-stuff but keeps count of items.
: ,init s" marker _dstack_ " evaluate 0 , ; \ initialize counter to zero.
: ,reset s" _dstack_" evaluate ; \ recovers memory, deletes dstack.
: ,depth here- @ ;
: ,. ( m -- ) here- dup @ 1+ -rot ! , ; \ push
: ,n 0 do ,. loop ;
: ,pop pop 1- here- dup @ -rot ! ;
: ,peekn 2 + cells negate here + @ ;
: ,npop 0 do ,pop loop ;
: ds>s ,depth ,npop ,reset ; \ move dstack to stack and delete dstack.
: s>ds ( j*x -- ) ,init depth 0 do ,. loop ; \ s>ds = save the stack to dstack.
: ns>ds ( i*x n -- j*x ) ,init 0 do ,. loop ;
\ : ,. pop 1+ swap , , ;
\ : ,pop pop 1- pop swap , ; \ pop
...
\ : ,buffer ( <name> -- ) 
	\ ,depth dup cells allocate drop 
	\ >r dup >r 0 do i peekn loop r> cells r@ + r> do i ! cell +loop ;
\ : ,restore ( addr -- ) ;
0 value _dic_start_
: mrk_dic here to _dic_start_ ;
mrk
